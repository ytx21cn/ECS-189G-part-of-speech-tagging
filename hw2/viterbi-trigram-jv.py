...
# Usage: python2 ./viterbi-trigram-jv.py <hmmFile> <inputFile> > <outputFile>
	...
	for line in inputFile.read().splitlines():
		...
		for i in xrange(1, currentLineLen + 1):
			...
			for prevPrevTag, prevTag, currentTag in itertools.product(tags, tags, tags):
				...
				if ((prevPrevTag, prevTag, currentTag) in transProbs) and ((currentTag, word) in emitProbs) and ((i - 1, prevPrevTag, prevTag) in initProbs):
					viterbiProb = initProbs[(i - 1, prevPrevTag, prevTag)] + transProbs[(prevPrevTag, prevTag, currentTag)] + emitProbs[(currentTag, word)] # log of product

					# if we found a better previous state, take note!
					if ((i, prevTag, currentTag) not in initProbs) or (viterbiProb > initProbs[(i, prevTag, currentTag)]):
						# note that the indices here are at least 1
						initProbs[(i, prevTag, currentTag)] = viterbiProb
						backtrace[(i, prevTag, currentTag)] = prevPrevTag
